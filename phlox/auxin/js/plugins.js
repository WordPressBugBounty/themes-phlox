...
;(function ($) {
	...
	Plugin.prototype = {
		...
		init: function () {
            ...
            this.iconPicker[0].style.left = '-9999px';
            document.body.appendChild(this.iconPicker[0]);

			var iconPickerHeight = this.iconPicker.outerHeight(),
			iconPickerWidth = this.iconPicker.outerWidth();

			// Now reset the iconPicker CSS
			this.iconPicker[0].style.left = '';

			// Add the icon picker after the select
            this.element[0].parentNode.insertBefore(this.iconPicker[0], this.element[0]);
			...
			if (!this.settings.source && this.element.is('select')) {
                ...
                const groups = this.element[0].querySelectorAll('optgroup');
                ...
                if (groups.length) {
                    ...
                    groups.forEach((el) => {
                        ...
                        categoryOption.value = thisCategoryKey;
                        // Set the label
                        categoryOption.innerHTML = el.getAttribute('label');

                        // Append to the DOM
                        this.selectCategory[0].appendChild(categoryOption);

                        // Init the availableCategories array
                        this.availableCategories[thisCategoryKey] = [];
                        this.availableCategoriesSearch[thisCategoryKey] = [];

                        // Now loop through its option elements and add the icons
                        el.querySelectorAll('option').forEach((cel) => {
                            var newIconValue = cel.value,
                            newIconLabel = cel.innerHTML;
                    ...
                    const options = this.element[0].querySelectorAll('option');

                    // Additionally check for any first label option child
                    const topLevelOptions = this.element[0].querySelectorAll('> option');
                    if (topLevelOptions.length) {
                        topLevelOptions.forEach((el) => {
                            var newIconValue = el.value,
                            newIconLabel = el.innerHTML;
                            ...
                            if (this.unCategorizedKey === null) {
                                ...
                                uncategorizedOption.value = this.unCategorizedKey;
                                uncategorizedOption.innerHTML = this.settings.unCategorizedText;
                                this.selectCategory[0].appendChild(uncategorizedOption);
                            }
				...
				// Not categorized
                    ...
                    this.element[0].querySelectorAll('option').forEach((el) => {
                        var newIconValue = el.value,
                        newIconLabel = el.innerHTML;
                        if (newIconValue) {
                            this.settings.source.push(newIconValue);
		 ...
		 * Load icons
		 */
		loadIcons: function (renderList = true) {

            // Set the content of the popup as loading
            this.iconContainer[0].innerHTML = '<i class="fip-icon-spin3 animate-spin loading"></i>';

            // If source is set
            if (Array.isArray(this.settings.source)) {

                // Render icons
                this.renderIconContainer( renderList);
		 ...
		 */
		renderIconContainer: function ( renderList = true ) {
            ...
            if (renderList) {
                ...
                if (this.totalPage > 1) {
                    ...
                    this.iconPicker[0].querySelector('.selector-footer').style.display = 'block';
                } else {
                    // this.iconPicker.find('.selector-footer').hide();
                    this.iconPicker[0].querySelector('.selector-footer').style.display = 'none';
                }
                ...
                this.iconPicker[0].querySelector('.selector-pages').innerHTML = this.currentPage + '/' + this.totalPage + ' <em>(' + this.iconsCount + ')</em>';
            ...
            if (renderList) {
                ...
                for (var i = 0, item; (item = iconsPaged[i++]); ) {
                    ...
                    var index = this.settings.source.indexOf(item);
                    if (index !== -1) {
                        flipBoxTitle = this.searchValues[index];
                    }

                    // Set the icon box
                    var span = document.createElement("span");
                    span.className = "fip-box";
                    span.title = flipBoxTitle;

                    var iElement = document.createElement("i");
                    iElement.setAttribute("data-fip-value", item);
                    if (this.settings.useAttribute) {
                        var attrValue = this.settings.convertToHex
                            ? "&#x" + parseInt(item, 10).toString(16) + ";"
                            : item;
                        iElement.setAttribute(this.settings.attributeName, attrValue);
                    } else {
                        iElement.className = item;
                    }
                    span.appendChild(iElement);

                    this.iconContainer[0].appendChild(span);
                }
            }
